...
class Vector3DToAxisAngleMLP(nn.Module):
    """
    MLP network mapping 3D vectors to axis-angle representation
    Input: 3D vector [x, y, z]
    Output: axis-angle representation [axis_x, axis_y, axis_z, angle], where axis is unit vector, angle in [0, π] range
    """
    ...
    def forward(self, x):
        ...
        axis_raw = raw_output[:, :3]  # first 3 dimensions are axis
        angle_raw = raw_output[:, 3:4]  # 4th dimension is angle
        
        axis_norm = torch.norm(axis_raw, dim=1, keepdim=True)
        axis_norm = torch.clamp(axis_norm, min=1e-8)
...
def vector_to_axis_angle_analytical(vectors):
    """
    Analytical method: convert 3D vectors to axis-angle representation
    Here we define a mapping rule: vector direction as rotation axis, vector length mapped to rotation angle
    """
...
def axis_angle_to_rotation_matrix(axis_angle):
    """
    Convert axis-angle representation to rotation matrix (for verification)
...
class AxisAngleTrainer:
    ...
    def custom_loss_function(self, y_pred, y_true):
        """
        Custom loss function: considers geometric properties of axis and angle
        """
        # 分离预测和真实的轴角表示
        pred_axis = y_pred[:, :3]
        pred_angle = y_pred[:, 3]
